#!/usr/bin/env python3
"""DreamerV3 + DriftHead: learn to predict per-step imagination drift.

DriftHead is trained SEPARATELY from the main DreamerV3 loop:
- World model trains normally (no modification)
- Actor trains normally (no modification)  
- DriftHead trains on its own loss: predict (imagined_deter - posterior_deter)

At inference time, DriftHead correction is subtracted from img_step output.
"""

from __future__ import annotations
import argparse, functools, pathlib, sys, types, json
...
def main():
    ...
    # Print banner
    print("=" * 70)
    print("DreamerV3 + DriftHead Training")
    print("=" * 70)
    print(f"Task: {config.task}")
    print(f"Steps: {config.steps} (env steps: {config.steps * config.action_repeat})")
    print(f"Device: {config.device}")
    print(f"Seed: {config.seed}")
    print(f"DriftHead horizon: {run_config.get('drift_head', {}).get('train_horizon', 15)}")
    print("=" * 70, flush=True)
    ...
    # DriftHead gets its OWN optimizer (independent of actor/model)
    ...
    # Force gate open for drift head (we control magnitude via the learned MLP)
    with torch.no_grad():
    ...
    # Patch WorldModel._train to add drift_head training step
    original_wm_train = agent._wm._train
    
    def wm_train_with_drift_head(wm_self, data):
        # 1. Normal world model training (completely unchanged)
        post, context, metrics = original_wm_train(data)
        
        # 2. DriftHead training step (independent, doesn't affect WM)
    ...
    # Training loop (standard DreamerV3)
...
def train_drift_head_step(*, dynamics, post, actions, drift_head, optimizer, horizon, torch_module):
    """Train drift_head to predict per-step imagination drift.
    
    For each batch element, start from a random posterior state, 
    run img_step for `horizon` steps, and train drift_head to predict
    the drift (imagined_deter - posterior_deter) at each step.
    """
    import torch
    
    # post is {deter: (B, T, D), stoch: (B, T, S1, S2), ...}
    # or after observe: {deter: (T, B, D), ...} — need to check
    post_deter = post["deter"].detach()  # (T, B, D) or (B, T, D)
    ...
    actions_det = actions if isinstance(actions, torch.Tensor) else torch.tensor(actions, device=post_deter.device)
    
    # Pick random start indices that leave room for `horizon` steps
    ...
    # Initialize imagination from posterior at start_idx
    state = {k: v[start_idx].detach() for k, v in post.items()}
    
    total_loss = torch.tensor(0.0, device=post_deter.device)
    n_steps = 0
    ...
    for k in range(horizon):
        ...
        # One-step imagination (detach input to prevent chain)
        with torch.no_grad():
            next_state = dynamics.img_step(state, action, sample=False)
        
        imagined_deter = next_state["deter"].detach()  # (B, D)
        posterior_deter = post_deter[t]  # (B, D)
        
        # True drift at this step
        true_drift = imagined_deter - posterior_deter  # (B, D)
        
        # DriftHead predicts the correction (WITH gradient)
        predicted_correction = drift_head(imagined_deter, step=k)
        
        # Loss: predicted correction should match true drift
        step_loss = torch.nn.functional.mse_loss(predicted_correction, true_drift.detach())
        total_loss = total_loss + step_loss
        n_steps += 1
        
        # Advance state (detached, no chain)
...
def parse_args():
    ...
    p.add_argument("--run_name", default="month8_drift_head")
