...
# Unified V3 same-pod baseline/SHARP launcher.
#
# Usage:
#   bash scripts/runpod_v3_multiseed.sh 43
#   bash scripts/runpod_v3_multiseed.sh 43 cheetah
#   bash scripts/runpod_v3_multiseed.sh 43 cartpole

if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
  echo "Usage: bash scripts/runpod_v3_multiseed.sh SEED [cheetah|cartpole]"
  exit 2
fi
...
ROOT="results/v3_multiseed"
BASELINE_RUN_NAME="v3_multiseed/${TASK_SHORT}_baseline_seed${SEED}"
SHARP_RUN_NAME="v3_multiseed/${TASK_SHORT}_sharp_seed${SEED}"
...
SHARP_CONFIG="${CONFIG_DIR}/sharp.yaml"
...
if not torch.cuda.is_available():
    raise SystemExit("CUDA is required for V3 multiseed training.")
...
generate_configs() {
  python - "${SEED}" "${BASE_CONFIG_PATH}" "${BASELINE_CONFIG}" "${SHARP_CONFIG}" <<'PY'
import copy
import sys
from pathlib import Path
...
sharp_path = Path(sys.argv[4])
yaml = YAML()
yaml.default_flow_style = False

base = yaml.load(base_config.read_text())
base["seed"] = seed

baseline = copy.deepcopy(base)
sharp = copy.deepcopy(base)

baseline["moment_match"]["beta_mean"] = 0.0
baseline["moment_match"]["beta_var"] = 0.0

baseline_path.parent.mkdir(parents=True, exist_ok=True)
with baseline_path.open("w") as handle:
    yaml.dump(baseline, handle)
with sharp_path.open("w") as handle:
    yaml.dump(sharp, handle)
PY
}
...
print_config_diff() {
  ...
  echo "SHARP config:     ${SHARP_CONFIG}"
  echo "Baseline vs SHARP config diff:"
  diff -u "${BASELINE_CONFIG}" "${SHARP_CONFIG}" || true
}
...
train_baseline() {
  phase "${TASK_SHORT} baseline training seed ${SEED}"
  run_nohup_and_wait \
    "V3 ${TASK_SHORT} baseline seed ${SEED}" \
    "${LOG_DIR}/baseline_train_${RUN_ID}.log" \
    python scripts/train_moment_match.py \
...
train_sharp() {
  phase "${TASK_SHORT} SHARP training seed ${SEED}"
  run_nohup_and_wait \
    "V3 ${TASK_SHORT} SHARP seed ${SEED}" \
    "${LOG_DIR}/sharp_train_${RUN_ID}.log" \
    python scripts/train_moment_match.py \
      --config "${SHARP_CONFIG}" \
...
extract_rollouts() {
  phase "${TASK_SHORT} rollout extraction seed ${SEED}"
  run_logged \
    "${TASK_SHORT} baseline rollout extraction seed ${SEED}" \
    "${LOG_DIR}/baseline_extract_${RUN_ID}.log" \
    python scripts/batch_extract_v3_multiseed.py \
      --checkpoint "${BASELINE_CKPT}" \
      --output_dir "${BASELINE_DIR}/rollouts" \
      --task "${DMC_TASK}" \
      ...
      --manifest_name "manifest_${TASK_SHORT}_baseline_seed${SEED}.json"

  run_logged \
    "${TASK_SHORT} SHARP rollout extraction seed ${SEED}" \
    "${LOG_DIR}/sharp_extract_${RUN_ID}.log" \
    python scripts/batch_extract_v3_multiseed.py \
      --checkpoint "${SHARP_CKPT}" \
      --output_dir "${SHARP_DIR}/rollouts" \
      --task "${DMC_TASK}" \
      --num_seeds "${N_ROLLOUT_SEEDS}" \
      --total_steps "${TOTAL_ROLLOUT_STEPS}" \
      --imagination_start "${IMAGINATION_START}" \
      --horizon "${HORIZON}" \
      --device "${DEVICE_EXTRACT}" \
      --manifest_name "manifest_${TASK_SHORT}_sharp_seed${SEED}.json"
  mark_done "${CURRENT_PHASE}"
}

list_outputs() {
  echo "V3 multiseed outputs for ${TASK_SHORT} seed ${SEED}:"
...
extract_rollouts
list_outputs
echo "=== V3 ${TASK_SHORT} multiseed seed ${SEED} complete at $(date) ==="
