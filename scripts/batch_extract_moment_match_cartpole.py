"""Batch rollout extraction for DreamerV3 v2 latent-aware rollouts.
...
CHECKPOINT = REPO_ROOT / "results" / "month8_moment_match_cartpole" / "latest.pt"
OUTPUT_DIR = REPO_ROOT / "results" / "month8_controlled" / "rollouts_moment_match_cartpole"
MANIFEST = OUTPUT_DIR / "manifest_moment_match_cartpole.json"
...
def update_manifest(adapter: DreamerV3Adapter) -> dict[str, object]:
    files = [OUTPUT_DIR / f"cheetah_v3_seed{seed}_v2.npz" for seed in SEEDS]
...
def main() -> None:
    ...
    for seed in SEEDS:
        output = OUTPUT_DIR / f"cheetah_v3_seed{seed}_v2.npz"
        label = f"{seed + 1}/20"
        if output.exists():
            print(
